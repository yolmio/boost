import { ImageSetFieldGroup, Table } from "../../system";
...
function createUndoSnackbar() {
  ...
  const undidSnackbar = system.currentApp!.registerCrossPageSnackbar({
  ...
  const undoFailure = system.currentApp!.registerCrossPageSnackbar({
    autoHideDuration: 3_000,
    componentOpts: (close) => ({
      variant: "soft",
      color: "danger",
  ...
  return system.currentApp!.registerCrossPageSnackbar({
    componentOpts: (close) => ({
      variant: "soft",
      color: "success",
